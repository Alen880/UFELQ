...
class HDRCC_2_share(BaseModel):
    ...
    def backward_G(self):
        ...
        self.loss_G_Content = self.contentLoss.get_loss(self.real_B * self.input_region,
                                                        self.fake_B * self.input_region)  #
        self.loss_G_Segmen = self.segmenLoss.get_loss(self.real_B_ves * self.input_region,
                                                      self.fake_B_ves * self.input_region) * self.opt.lambda_S  # * self.input_region
        self.loss_G_Disk_Content = self.contentLoss.get_loss(self.real_B * self.input_disk,
                                                             self.fake_B * self.input_disk) * 10

        # sacle 2
        self.loss_G_Content_256 = self.contentLoss.get_loss(self.real_B_256 * self.input_region_256,
                                                            self.fake_B_256 * self.input_region_256)  #
        self.loss_G_Disk_Content_256 = self.contentLoss.get_loss(self.real_B_256 * self.input_disk_256,
                                                                 self.fake_B_256 * self.input_disk_256) * 10
    ...
    def get_current_visuals(self):
        ...
        return OrderedDict([('Blurred_Train', real_A), ('Restored_Train', fake_B), ('Sharp_Train', real_B),
                            ('Restored_Train_256', fake_B_256), ('Sharp_Train_256', real_B_256),
                            ('Restored_vessel', real_A), ('sharp_vessel', real_A)])
