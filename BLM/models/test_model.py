...
class TestModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if opt.which_model_netG == 'encode_decode_vessel_2scale_share_res':
            ...
            self.input_256 = self.Tensor(opt.batchSize, opt.input_nc, np.int(opt.fineSize / 2),
                                         np.int(opt.fineSize / 2))
        ...
        # self.input_A = self.Tensor(opt.batchSize, opt.input_nc, opt.fineSize, opt.fineSize)
            ...
            self.input_512 = self.Tensor(opt.batchSize, opt.input_nc, opt.fineSize, opt.fineSize)
            self.input_256 = self.Tensor(opt.batchSize, opt.input_nc, int(opt.fineSize / 2), int(opt.fineSize / 2))
            self.input_norm = self.Tensor(opt.batchSize, opt.input_nc, opt.fineSize, opt.fineSize)
    ...
    def set_input(self, input):
        ...
        if (self.opt.which_model_netG == 'encode_decode_vessel_2scale_share_res' or
                self.opt.which_model_netG == 'encode_decode_vessel_2scale_share_res_atten' or
                self.opt.which_model_netG == 'encode_decode_vessel_2scale_share_res_atten_sum'):
            input_512 = input['A']
    ...
    def test(self):
        ...
        elif (self.opt.which_model_netG == 'encode_decode_vessel_2scale_share_res_atten'
              or self.opt.which_model_netG == 'encode_decode_vessel_2scale_share_res_atten_sum'):
            self.real_A = Variable(self.input_512, requires_grad=False)
            self.real_A_256 = Variable(self.input_256, requires_grad=False)
            self.real_A_ves = Variable(self.input_norm, requires_grad=False)
            self.fake_B, self.fake_B_256, self.vess, self.spot = self.netG.forward(self.real_A, self.real_A_256,
                                                                                   self.real_A_ves)
    ...
    def get_current_visuals(self):
        if self.opt.which_model_netG == 'encode_decode_vessel_2scale_share_res':
            ...
            vessel = util.tensor2im(self.vess.data)
            return OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('vessel', vessel)])
        elif (self.opt.which_model_netG == 'encode_decode_vessel_2scale_share_res_atten' or
              self.opt.which_model_netG == 'encode_decode_vessel_2scale_share_res_atten_sum'):
            real_A = util.tensor2im(self.real_A.data)
            fake_B = util.tensor2im(self.fake_B.data)
            vessel = util.tensor2im(self.vess.data)
